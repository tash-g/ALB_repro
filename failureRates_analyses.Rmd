...
# Decide whether to run full analyses or not
...
packages <- c("dplyr", "magrittr", "ggplot2", "brms", "tidyverse", "tidybayes",
              "ggpubr", "extrafont", "cowplot", "performance", "kableExtra", "GGally")
...
failure_summary <- failure %>%
  ...
  summarise(
    Observations = n(),
    "Egg failed" = round(sum(rs == "FAILED_EGG")/Observations*100, 2),
    "Chick failed" = round(sum(rs == "FAILED_CHICK")/Observations*100, 2),
    "Failure stage unknown" = round(sum(rs == "FAILED")/Observations*100, 2),
    Successful = round(sum(rs == "SUCCESSFUL")/Observations*100, 2)) %>%
...
$\text{logit}(p_i) = \beta_0 + \beta_1 F_i + \beta_2 T_i + \beta_3 D_i + \beta_4 V_i + \beta_5 C_i + C_i (\beta_6 F_i + \beta_7 T_i + \beta_8 D_i + \beta_9 V_i) + U_i^{\text{season}}$

where:
- $F_i$ is the fasting debt,
- $T_i$ is the pair-level median trip duration,
- $D_i$ is the within-pair trip duration difference,
- $V_i$ is the within-pair trip variability difference,
- $C_i$ is the colony (Bird Island, Kerguelen, or Possession Island), and
- $U_i^{\text{season}} \sim \mathcal{N}(0, \sigma_s^2)$ represents the random effect of season, accounting for inter-annual variation in breeding success.
...
TableS3 <- rbind(vif_incub.bba, vif_brooding.bba, vif_incub.waal, vif_brooding.waal) 
...
TableS3 %>%
  kable("html", row.names = FALSE,
        caption = "Table S3. Variance Inflation Factors (VIFs) for fixed effects in the breeding success models used in the analyses. Values < 5 indicate low multicollinearity. VIF estimates include 95% confidence intervals. BBAL = black-browed albatross; WAAL = wandering albatross.") %>%
...
posterior_samples.incub_bba <- as_draws_df(incub_brms.bba)
...
posterior_samples.incub_waal <- as_draws_df(incub_brms.waal)
...
posterior_samples.brooding_waal <- as_draws_df(brooding_brms.waal)

# Extract posterior samples by colony
predictors <- colnames(posterior_samples.brooding_waal)[2:6]
predictors <- predictors[-2]
interaction_terms <- colnames(posterior_samples.brooding_waal)[7:10]
...
new_names <- c(
  ...
  "Within-pair trip duration difference",
  "Within-pair trip variability difference",
  "Fasting debt : Colony",
  "Pair-level median trip duration : Colony",
  "Within-pair trip duration difference : Colony",
  "Within-pair trip variability difference : Colony",
  "Fasting debt (Kerguelen)",
  "Pair-level median trip duration (Kerguelen)",
  "Within-pair trip duration difference (Kerguelen)",
  "Within-pair trip variability difference (Kerguelen)"
)

rownames(table1) <- new_names

## Get seasonal effects
bbal_incub.season.row <- summary(incub_brms.bba)$random$season %>%
  as.data.frame() %>%
  mutate(`Estimate ± Est.Error` = paste0(round(Estimate, 2), " \u00B1 ", round(Est.Error, 2)),
         `95% CIs` = paste0("[", round(`l-95% CI`, 2), ", ", round(`l-95% CI`, 2), "]"),
          `Inside ROPE` = "") %>%
  select(`Estimate ± Est.Error`, `95% CIs`, `Inside ROPE`)

bbal_brooding.season.row <- summary(brooding_brms.bba)$random$season %>%
  as.data.frame() %>%
  mutate(`Estimate ± Est.Error` = paste0(round(Estimate, 2), " \u00B1 ", round(Est.Error, 2)),
         `95% CIs` = paste0("[", round(`l-95% CI`, 2), ", ", round(`l-95% CI`, 2), "]"),
          `Inside ROPE` = "") %>%
...
#readr::write_excel_csv(table1, "Data_outputs/table1_bbal_coefficients.csv", row.names = T)
...
new_names <- c(
  ...
  "Within-pair trip duration difference (crozet)",
...
## Get seasonal effects
waal_incub.season.row <- summary(incub_brms.waal)$random$season %>%
  as.data.frame() %>%
  mutate(`Estimate ± Est.Error` = paste0(round(Estimate, 2), " \u00B1 ", round(Est.Error, 2)),
         `95% CIs` = paste0("[", round(`l-95% CI`, 2), ", ", round(`l-95% CI`, 2), "]"),
          `Inside ROPE` = "") %>%
...
waal_brooding.season.row <- summary(brooding_brms.waal)$random$season %>%
  as.data.frame() %>%
  mutate(`Estimate ± Est.Error` = paste0(round(Estimate, 2), " \u00B1 ", round(Est.Error, 2)),
         `95% CIs` = paste0("[", round(`l-95% CI`, 2), ", ", round(`l-95% CI`, 2), "]"),
          `Inside ROPE` = "") %>%
...
save(cond_plot.median_trip.days.bba_incub, file = "Data_outputs/cond_plot.median_trip.days.bba_incub.RData")
save(cond_plot.median_trip.days.bba_brooding, file = "Data_outputs/cond_plot.median_trip.days.bba_brooding.RData")
save(cond_plot.diff_trip.days.bba_brooding,file = "Data_outputs/cond_plot.diff_trip.days.bba_brooding.RData")
save(cond_plot.diff_var.days.waal_incub,file = "Data_outputs/cond_plot.diff_var.days.waal_incub.RData")
save(cond_plot.median_trip.days.waal_brooding,file = "Data_outputs/cond_plot.median_trip.days.waal_brooding.RData")
save(cond_plot.diff_trip.days.waal_brooding,file = "Data_outputs/cond_plot.diff_trip.days.waal_brooding.RData")

```

```{r Figure2_bbal_condiitonal_effects, fig.width = 10, fig.height = 10, fig.cap = "**Figure 2.** Conditional effect estimates of (A) pair-level median trip duration during incubation, and (B) pair-level median trip duration and (C) within-pair trip duration difference during brooding on breeding success of black-browed albatrosses. Effects are shown separately for Bird Island (orange) and Kerguelen (blue). Solid lines represent the estimated mean effect, and the shaded areas denote the 95% credible interval. Data points correspond to individual pair estimates. Note that some estimates may extend outside the range of observed data due to extrapolation and should thus be interpreted with caution."}

load("Data_outputs/cond_plot.median_trip.days.bba_incub.RData")
load("Data_outputs/cond_plot.median_trip.days.bba_brooding.RData")
load("Data_outputs/cond_plot.diff_trip.days.bba_brooding.RData")
...
```{r Figure3_waal_condiitonal_effects, fig.width = 10, fig.height = 10, fig.cap = "**Figure 3.** Conditional effect estimates of (A) within-pair trip variability difference during incubation, and (B) pair-level median trip duration and (C) within-pair trip duration difference during brooding on breeding success of wandering albatrosses. Effects are shown separately for Bird Island (orange) and Crozet (blue). Solid lines represent the estimated mean effect, and the shaded areas denote the 95% credible interval. Data points correspond to individual pair estimates."}

load("Data_outputs/cond_plot.diff_var.days.waal_incub.RData")
load("Data_outputs/cond_plot.median_trip.days.waal_brooding.RData")
load("Data_outputs/cond_plot.diff_trip.days.waal_brooding.RData")
...
The x-fold changes are calculated by exponentiating the effect sizes from the Bernoulli model, which operates on the log-odds scale. The change in probability is determined by converting the log-odds to probabilities at different values of 'X' and calculating the difference between them. For negative effects, the x-fold change is calculated as 1/exponentiated effect.
...
# Brooding
behav_cor_bba.brooding <- ggpairs(
  data = behaviour.bba_brooding %>%
    select(colony, debt.days, median_trip.days, diff_trip.days, diff_var.days),
  columns = 2:5,
  mapping = ggplot2::aes(colour = colony),
